...
namespace PixelCrushers.QuestMachine
{
    ...
    public class PlanToQuestBuilder
    {
            ...
            out string counterName, out int requiredCounterValue, PlanStep goal, PlanStep step)
        {
            ...
            if (completion.mode == ActionCompletion.Mode.Counter)
            {
                ...
                if (!counterNames.Contains(counterName))
                {
                    ...
                    foreach (var messageEvent in completion.messageEventList)
                    {
                        ...
                        counterMessageEvent.senderSpecifier = GetSpecifier(step.action.completion.senderSpecifier, step.action.completion.senderID);
                        counterMessageEvent.targetSpecifier = GetSpecifier(step.action.completion.targetSpecifier, step.action.completion.targetID);
                        counterMessageEvent.senderID = step.action.completion.senderID;
                        counterMessageEvent.targetID = step.action.completion.targetID;
            ...
            else
            {
                ...
                var parameter = StringField.GetStringValue(completion.parameter).
                    Replace("{TARGETENTITY}", targetEntity).
                    Replace("{DOMAIN}", domainName);
                var senderSpecifier = completion.senderSpecifier;
                ...
                var targetSpecifier = completion.targetSpecifier;
        ...
        protected virtual QuestMessageParticipant GetSpecifier(StringField specifierID)
        {
            return GetSpecifier(QuestMessageParticipant.Other, specifierID);
        }

        protected virtual QuestMessageParticipant GetSpecifier(QuestMessageParticipant specifier, StringField specifierID)
        {
            if (specifier != QuestMessageParticipant.Other)
            {
                return specifier;
            }
            else if (StringField.IsNullOrEmpty(specifierID))
            {
                return QuestMessageParticipant.Any;
            }
            else if (specifierID.value == QuestMachineTags.QUESTER)
            {
                return QuestMessageParticipant.Quester;
            }
            else if (specifierID.value == QuestMachineTags.QUESTGIVERID)
            {
                return QuestMessageParticipant.QuestGiver;
            }
            else
            {
                return QuestMessageParticipant.Other;
            }
        }
