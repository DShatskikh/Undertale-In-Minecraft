...
namespace PixelCrushers.QuestMachine
{
    ...
    public class UnityUIIconListTemplate : UnityUIContainerTemplate
    {
        ...
        public UnityUIIconTemplate AddIcon(IconQuestContent icon)
        {
            ...
            var captionTextParent = instance.captionText.gameObject.transform.parent;
            if (captionTextParent != this.transform)
            {
                captionTextParent.gameObject.SetActive(!string.IsNullOrEmpty(icon.runtimeText));
            }
            return instance;
        }
