...
namespace PixelCrushers.QuestMachine
{
    ...
    public class UnityUIQuestJournalUI : UnityUIBaseUI, IQuestJournalUI
    {
        ...
        private bool m_justToggledTracking = false;
        ...
        protected virtual void AddQuestToUI(Quest quest, RectTransform container)
        {
            ...
            var target = quest;
            SetupQuestNameUIEvents(questName.buttonTemplate.button, target);
            if ((showFirstQuestDetailsOnOpen && m_justShown) || (quest == selectedQuest))
            {
                if (m_justToggledTracking)
                {
                    m_justToggledTracking = false;
                    UIUtility.Select(questName.trackToggleTemplate.toggle);
                }
                else
                {
