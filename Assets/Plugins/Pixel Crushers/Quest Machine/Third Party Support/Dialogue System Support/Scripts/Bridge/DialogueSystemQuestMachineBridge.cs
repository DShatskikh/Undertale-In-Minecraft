    ...
    public class DialogueSystemQuestMachineBridge : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            else
            {
                ...
                Lua.RegisterFunction("ShowQuestConversation", this, SymbolExtensions.GetMethodInfo(() => ShowQuestConversation(string.Empty, string.Empty, string.Empty)));
                Lua.RegisterFunction("ShowGeneratedQuestConversation", this, SymbolExtensions.GetMethodInfo(() => ShowGeneratedQuestConversation(string.Empty, string.Empty)));
        ...
        private void SetQuesterQuestNodeState(string questID, string questNodeID, string state, string questerID)
        {
            ...
            var newState = string.Equals(state, QuestLog.StateToString(DialogueSystem.QuestState.Active)) ? QuestNodeState.Active
                : (string.Equals(state, QuestLog.StateToString(DialogueSystem.QuestState.Success)) || (string.Equals(state, "true", System.StringComparison.OrdinalIgnoreCase))) ? QuestNodeState.True
                : QuestNodeState.Inactive;
        ...
        /// <summary>
        /// Switch to the specified quest conversation.
        /// </summary>
        public void ShowQuestConversation(string questGiverID, string questID, string questerID)
        {
            var questGiver = GetQuestGiver(questGiverID);
            var quester = GetQuester(questerID);
            if (questGiver == null || quester == null) return;
            if (!TryShowQuestConversation(questGiver, questID, quester, questGiver.questList))
            {
                TryShowQuestConversation(questGiver, questID, quester, quester.questList);
            }
        }

        protected bool TryShowQuestConversation(QuestGiver questGiver, string questID, QuestJournal quester, List<Quest> questList)
        {
            if (questGiver == null || quester == null || questList == null) return false;
            foreach (Quest quest in questList)
            {
                if (string.IsNullOrEmpty(questID) || StringField.Equals(quest.id, questID))
                {
                    var questerCopy = quester.FindQuest(quest.id);
                    var questState = quest.GetState();
                    var canShowQuestDialogue =
                        (questState == QuestState.WaitingToStart && quest.timesAccepted < quest.maxTimes) || 
                        (questerCopy != null);
                    if (!canShowQuestDialogue) continue;
                    DialogueManager.StopAllConversations();
        ...
        /// </summary>
        public void ShowGeneratedQuestConversation(string questGiverID, string questerID)
        {
            var questGiver = GetQuestGiver(questGiverID);
            var quester = GetQuester(questerID);
            if (questGiver == null || quester == null) return;
            if (!TryShowGeneratedQuestConversation(questGiver, quester, questGiver.questList))
            {
                TryShowGeneratedQuestConversation(questGiver, quester, quester.questList);
            }
        }

        protected bool TryShowGeneratedQuestConversation(QuestGiver questGiver, QuestJournal quester, List<Quest> questList)
        {
            if (questGiver == null || quester == null || questList == null) return false;
            foreach (Quest quest in questList)
            {
