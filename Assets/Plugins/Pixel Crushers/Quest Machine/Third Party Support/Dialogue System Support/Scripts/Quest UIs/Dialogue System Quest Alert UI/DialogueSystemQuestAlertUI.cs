...
namespace PixelCrushers.QuestMachine.DialogueSystemSupport
{
    ...
    public class DialogueSystemQuestAlertUI : MonoBehaviour, IQuestAlertUI
    {
        ...
        [SerializeField] private bool m_queueAlertIfConversationActive = false;

        public bool queueAlertIfConversationActive
        {
            get { return m_queueAlertIfConversationActive; }
            set { m_queueAlertIfConversationActive = value; }
        }
        ...
        {
            if (queueAlertIfConversationActive && DialogueManager.isConversationActive)
            {
                DialogueLua.SetVariable("Alert", message);
            }
            else
            {
