...
namespace PixelCrushers.QuestMachine.DialogueSystemSupport
{
    ...
    public class DialogueSystemQuestDialogueUI : MonoBehaviour, IQuestDialogueUI
    {
        ...
        [Tooltip("When starting offer dialogue about a quest with no offer text, generate a conversation. If unticked, use Quest Dialogue UI originally assigned to Quest Machine.")]
        [SerializeField]
        private bool m_generateConversationForQuestWithNoOfferText = true;
        ...
        /// <summary>
        /// When starting offer dialogue about a quest with no offer text, generate a conversation.
        /// If false, use Quest Dialogue UI originally assigned to Quest Machine.
        /// </summary>
        public bool generateConversationForQuestWithNoOfferText
        {
            get { return m_generateConversationForQuestWithNoOfferText; }
            set { m_generateConversationForQuestWithNoOfferText = value; }
        }
        ...
        public virtual void ShowContents(QuestParticipantTextInfo speaker, List<QuestContent> contents)
        {
            ...
            {
                if (generateConversationForQuestWithNoOfferText && IsContentsJustBody(contents))
                {
        ...
        {
            if (generateConversationForQuestLists &&
                (DoesQuestListHaveConversationContent(offerableQuests, true) ||
                 DoesQuestListHaveConversationContent(activeQuests, false)))
            {
                selectedQuest = null;
        ...
        protected virtual bool DoesQuestListHaveConversationContent(List<Quest> questList, bool checkOfferText)
        {
            if (questList == null) return false;
            foreach (var quest in questList)
            {
                var contents = checkOfferText ? quest.offerContentList
                    : quest.GetContentList(QuestContentCategory.Dialogue);
                if (contents.Find(x => x is DialogueSystemConversationQuestContent) != null)
                {
                    return true;
                }
            }
